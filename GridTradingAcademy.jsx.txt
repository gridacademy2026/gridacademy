...
export default function GridAcademy() {
  ...
  const callAI = async (messages, system) => {
    // Secure server-side call — API key hidden in Vercel environment variables
    const r = await fetch("/api/gemini", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ messages, system })
    });
    if (!r.ok) throw new Error("AI unavailable. Please try again in a moment.");
    const d = await r.json();
    return d.text || "No response from AI.";
  };
